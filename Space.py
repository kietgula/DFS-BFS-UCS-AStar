...
class Node:
    def __init__(self, x, y, value, radius = 10) -> None:
        '''
        x,y: tọa độ của node
        value: giá trị của node, có thể dùng làm giá trị định danh -> dùng để làm chi phí từ các node bên cạnh đi tới node này
...
class Graph:
    ...
    def get_neighbors_bigger(self, node:Node):
        '''
        trả về các node kề của node `node` theo 8 hướng
        '''
        r = node.value//(cols-2)
        ...
        directions = [right,down_left,down,down_right]
        neighbors = []
        for dir in directions:
            if dir is not None:
                neighbors.append(self.grid_cells[dir[0]*(cols-2) + dir[1]])
    ...
    def get_neighbors_smaller(self, node:Node):
        '''
        trả về các node kề của node `node` theo 8 hướng
        '''
        r = node.value//(cols-2)
        ...
        directions = [left,up_right,up,up_left]
        neighbors = []
        for dir in directions:
            if dir is not None:
                neighbors.append(self.grid_cells[dir[0]*(cols-2) + dir[1]])
